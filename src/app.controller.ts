  ...
  health(): void {
    ...
    if (type === 1) {
      ...
      for (let j = 0; j < numNetworks; j++) {
        ...
        responses.push(
          entradas.map((entrada) => neuralNetwork.feedforward(entrada)[0]),
        );
      }
      ...
      for (let i = 0; i < maxIterations; i++) {
        ...
        if (best.error < errorThreshold) {
          ...
          console.log(
            'Expected(near): 0 ',
            lastBestNeuralNetwork.feedforward([3, 55])[0].toFixed(20),
          );
          console.log(
            'Expected(near): 1 ',
            lastBestNeuralNetwork
              .feedforward([-555555555, 555555557])[0]
              .toFixed(20),
          );
        ...
        if (lastErrorCount < errorStagnationThreshold) {
          ...
          responses.push(
            entradas.map((entrada) => newNetwork.feedforward(entrada)[0]),
          );
        }
        ...
        for (let j = 1; j < numNetworks; j++) {
          ...
          responses.push(
            entradas.map((entrada) => newNeuralNetwork.feedforward(entrada)[0]),
          );
        }
        lastBestNeuralNetwork = bestNeuralNetwork;
      }
      ...
      entradas.forEach((entrada) =>
        console.log(lastBestNeuralNetwork.feedforward(entrada)[0].toFixed(20)),
      );
    } else {
      const neuralNetwork = new NeuralNetwork(
        ActivationFunctionEnum.SIGMOID,
